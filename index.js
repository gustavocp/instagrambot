...
async function likeRandomPhotoFromHashtag(hashtag) {
    try {
        // Busca postagens recentes da hashtag
        const feed = ig.feed.tags(hashtag, 'recent');
        const posts = await feed.items();

        // Verifica se há postagens para a hashtag
        if (posts.length > 0) {
            const randomPost = posts[Math.floor(Math.random() * posts.length)];

            // Curte a postagem aleatória
            await ig.media.like({ mediaId: randomPost.id, d: 1 }); // Parâmetro "d" é obrigatório
            console.log(`Curtiu uma foto da hashtag #${hashtag}`);
            interactionCount++;
        } else {
            console.log(`Nenhuma postagem encontrada para a hashtag #${hashtag}`);
        }
    } catch (error) {
        console.error(`Erro ao curtir uma foto da hashtag #${hashtag}:`, error.message);
    }
}
...
async function runBot() {
  ...
  const actions = [
      async () => {
          ...
          await likeRandomPhotoFromHashtag(randomHashtag);
      },
    //   async () => {
    //       const randomHashtag = config.hashtagsToFollow[Math.floor(Math.random() * config.hashtagsToFollow.length)];
